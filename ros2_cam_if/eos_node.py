...
class EOS_node(Node):
    ...
    def __init__(self):
        ...
        self.get_ss_srv = self.create_service(Config, '~/get_shutterspeed', self.get_shutter_speed)
        self.set_ss_srv = self.create_service(Config, '~/set_shutterspeed', self.set_shutter_speed)
        self.get_iso_srv = self.create_service(Config, '~/get_iso', self.get_iso)
        self.set_iso_srv = self.create_service(Config, '~/set_iso', self.set_iso)
        self.set_contaf_srv = self.create_service(Config, '~/set_continuous_af', self.set_cont_af)
        self.get_imform_srv = self.create_service(Config, '~/get_image_format', self.get_imform)
        self.set_imform_srv = self.create_service(Config, '~/set_image_format', self.set_imform)
        self.set_afpoint_srv = self.create_service(Config, '~/set_af_point', self.set_afpoint)
    ...
    def set_aperture(self,request,response):
        current, choices, msg = self.cam.set_aperture(value=request.val1, list_choices=False)
        response.current_val = current
        response.choices = [str(choice) for choice in choices]
        response.output_msg = msg
        return response
    ...
    def set_shutter_speed(self,request,response):
        current, choices, msg = self.cam.set_shutterspeed(value=request.val1, list_choices=False)
        response.current_val = current
        response.choices = [str(choice) for choice in choices]
        response.output_msg = msg
        return response
    ...
    def set_iso(self,request,response):
        current, choices, msg = self.cam.set_iso(value=request.val1, list_choices=False)
        response.current_val = current
        response.choices = [str(choice) for choice in choices]
        response.output_msg = msg
        return response
    ...
    def set_cont_af(self,request,response):
        current, msg = self.cam.set_continuous_AF(value=request.val1)
        response.current_val = current
        response.output_msg = msg
        return response
    ...
    def set_imform(self,request,response):
        current, choices, msg = self.cam.set_image_format(value=request.val1, list_choices=False)
        response.current_val = current
